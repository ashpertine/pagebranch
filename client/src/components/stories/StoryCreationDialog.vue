  ...
  <v-dialog max-width="500" v-model="dialog">
    <v-card title="Add new story">
      ...
      <v-form class="px-5" @submit.prevent="makeStory(storyTitle)" ref="form">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text="Close" color="primary" size="large" @click="dialog = false"></v-btn>
          <v-btn variant="tonal" size="large" type="submit" append-icon="mdi-plus-circle" color="green">Add</v-btn>
        </v-card-actions>
      </v-form>
    </v-card>
  </v-dialog>
